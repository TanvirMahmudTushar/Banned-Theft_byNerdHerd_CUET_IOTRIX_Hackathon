...
async def get_today_meal_logs(db: Session = Depends(get_db)):
    ...
    for log in logs:
        ...
        # Find matching approval for extra info
        approval = db.query(models.ApprovalQueue).filter(
            models.ApprovalQueue.student_id == log.student_id,
            models.ApprovalQueue.requested_meal == log.meal_type,
            models.ApprovalQueue.status == "approved"
        ).order_by(models.ApprovalQueue.resolved_at.desc()).first()
        if student:
            result.append({
                "id": log.id,
                "student_id": log.student_id,
                "student_name": student.name,
                "student_id_number": approval.student_id_number if approval else None,
                "token_id": approval.token_id if approval else None,
